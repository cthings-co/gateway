                                ...
                                sd_bus_error * error)
{
	...
	for (int i = 0; i < neighbors_list.number_of_neighbors; i++) {
		r = sd_bus_message_open_container(reply, SD_BUS_TYPE_ARRAY, "(uuuuuuuuu)");
		r = sd_bus_message_open_container(reply, SD_BUS_TYPE_STRUCT, "uuuuuuuuu");
		r = sd_bus_message_append(reply, "u", (uint32_t)neighbors_list.nbors[i].add);
		r = sd_bus_message_append(reply, "u", (uint32_t)neighbors_list.nbors[i].link_rel);
		r = sd_bus_message_append(reply, "u", (uint32_t)neighbors_list.nbors[i].norm_rssi);
		r = sd_bus_message_append(reply, "u", (uint32_t)neighbors_list.nbors[i].cost);
		r = sd_bus_message_append(reply, "u", (uint32_t)neighbors_list.nbors[i].channel);
		r = sd_bus_message_append(reply, "u", (uint32_t)neighbors_list.nbors[i].nbor_type);
		r = sd_bus_message_append(reply, "u", (uint32_t)neighbors_list.nbors[i].tx_power);
		r = sd_bus_message_append(reply, "u", (uint32_t)neighbors_list.nbors[i].rx_power);
		r = sd_bus_message_append(reply, "u", (uint32_t)neighbors_list.nbors[i].last_update);
